"""
Pulled from flask restful for local modificaitons.
"""
...
def get_value(key, obj, default=None):
    ...
    if is_indexable_but_not_string(obj):
        try:
            ...
            return default
    return get_nested_value(key, obj, default)


def get_nested_value(key, obj, default):
    if isinstance(key, basestring):
        key = key.split('.')
    if hasattr(obj, key[0]):
        attr = getattr(obj, key.pop(0))
        if key:
            return get_nested_value(key, attr, default)
        return attr
    return default
