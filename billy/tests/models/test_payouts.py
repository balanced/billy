...
class TestCreate(TestPayout):
    def test_create(self):
        Payout.create(external_id='MY_TEST_PAYOUT',
                      group_id=self.group,
                      name='Payout',
...
class TestRetrieve(TestPayout):
    ...
    def test_retrieve_active_only(self):
        payout = Payout.create(external_id='MY_TEST_PAYOUT',
                               group_id=self.group,
                               name='Payout 2',
                               balance_to_keep_cents=123456,
                               payout_interval=Intervals.TWO_WEEKS
        ...
        payout.delete()
        with self.assertRaises(NoResultFound):
            Payout.retrieve('MY_TEST_PAYOUT', self.group, active_only=True)
...
class TestUpdateDelete(TestPayout):
    def test_update(self):
        ret = Payout.create(external_id='MY_TEST_PAYOUT',
                            group_id=self.group,
                            name='Payout 1',
                            balance_to_keep_cents=123456,
                            payout_interval=Intervals.TWO_WEEKS
        ...
        ret.update('new name')
        new = Payout.retrieve('MY_TEST_PAYOUT', self.group)
